...
func checkBingoForBoards(matched [][5][5]uint) int {
	ret := -1
...
func main() {
	...
	for _, v := range input {
		...
		printBoards(matched[:])
		bingoIndex := checkBingoForBoards(matched[:])
		if bingoIndex >= 0 {
			fmt.Println("Bingo", bingoIndex)
			fmt.Println("Winning Board:")
			printBoard(matched[bingoIndex])
			fmt.Println(getUnmarkedSum(boards[bingoIndex], matched[bingoIndex]) * v)
